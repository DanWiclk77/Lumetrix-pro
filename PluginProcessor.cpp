// ============================================================
//  LUMETRIX PRO – PluginProcessor.cpp
// ============================================================
#include "PluginProcessor.h"
#include "PluginEditor.h"

// ── K-weighting filter coefficients (48 kHz) ─────────────────
// Recalculated per EBU Tech 3341 / ITU-R BS.1770
...
{
    // If not 48 kHz, JUCE's IIR helper should recalculate.
    // For simplicity we use the standard 48 kHz coefficients here.
    // In production, compute via bilinear transform per sample rate.
    (void)sampleRate;
    std::copy(kPreB, kPreB+3, bPre);
    std::copy(kPreA, kPreA+3, aPre);
    std::copy(kRlbB, kRlbB+3, bRlb);
    std::copy(kRlbA, kRlbA+3, aRlb);
    std::fill(xPreL, xPreL+3, 0.0);
    std::fill(yPreL, yPreL+3, 0.0);
    std::fill(xPreR, xPreR+3, 0.0);
    std::fill(yPreR, yPreR+3, 0.0);
    std::fill(xRlbL, xRlbL+3, 0.0);
    std::fill(yRlbL, yRlbL+3, 0.0);
    std::fill(xRlbR, xRlbR+3, 0.0);
    std::fill(yRlbR, yRlbR+3, 0.0);
}
...
void LumetrixAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    // Momentary / short-term buffers (hold mean-square values)
                                          ...
                                          juce::MidiBuffer&)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        else peakHoldL = std::max(peakHoldL - 0.001f, -70.f); // slow decay
        ...
        double power = 0.5 * (kL*kL + kR*kR); // mean of stereo
        ...
        // Integrated gating (simplified: block = 100ms)
        gate.sumPower += power;
        gate.blockCount++;
